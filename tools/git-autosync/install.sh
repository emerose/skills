#!/bin/bash
# Build + install the git-autosync menu-bar app for one git working tree.
# All host-specific values come from flags — nothing is hard-coded.
#
# Usage:
#   ./install.sh --repo "/abs/path/to/working/tree" [options]
# Options:
#   --name NAME         menu-bar app name (default: GitSync). Pick a distinct
...
REPO="" NAME="GitSync" REMOTE="origin" BRANCH="main" INTERVAL="900"
while [[ $# -gt 0 ]]; do
  case "$1" in
    --repo) REPO="$2"; shift 2 ;;
    --name) NAME="$2"; shift 2 ;;
...
SWIFT="$SRCDIR/git-autosync.swift"
...
# 5. install the login agent. An Aqua-session app has native access to
#    provider-backed folders (~/Library/CloudStorage, …), so no Full Disk Access.
cat > "$PLIST" <<PLISTEOF
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
