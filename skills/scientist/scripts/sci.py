...
def _register_research_resolvers() -> bool:
    """Best-effort: import the ``research`` skill's literature citation layer so an experiment
    report that cites ``[lit:]`` / ``[litreview:]`` resolves at audit time. Importing
    ``research.literature_cites`` registers those schemes with the shared reportkit engine.

    scientist does NOT depend on research — this is the optional composition seam (the two skills
    meet only at the engine's citation registry). When research isn't installed this is a no-op and
    the engine surfaces a non-blocking ``unregistered-scheme`` warning instead of silently dropping
    the citation. Returns whether the resolvers were registered."""
    try:
        import research.literature_cites  # noqa: F401  (registers on import)
        return True
    except ImportError:
        return False
...
def _report(args: argparse.Namespace) -> int:
    ...
    # An experiment report may cite [lit:] / [litreview:]. scientist is literature-free, so the
    # resolvers come from the research skill IF installed (registered via reportkit on import) —
    # the optional composition seam. When research is absent the engine warns (unregistered-scheme)
    # rather than dropping the citation.
    _register_research_resolvers()
    ...
    if getattr(args, "write_pins", False):
        ...
        # litreview_pins so re-runs stay green until the survey's search protocol drifts. The
        # pin-writer is literature-specific (it lives in research); surfaced is non-empty only when
        # research's [litreview:] resolver ran, so the import below always succeeds in that branch.
        surfaced = {lc["id"]: lc["pin"] for lc in result.get("litreview_cites", [])
                    if lc.get("pin")}
        if surfaced:
            from research.literature_cites import write_litreview_pins
            merged = write_litreview_pins(path, surfaced)
            if not args.json:
            ...
            print("no litreview pins to write — no resolved [litreview:] cite "
                  "(install the research skill if this report cites [litreview:])")
