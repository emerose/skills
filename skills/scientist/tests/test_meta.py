...
from scientist.store import _meta


def test_parse_experiment_dirname_basic():
    p = _meta.parse_experiment_dirname("K1-000000 - Rat IT Dose-Response (V1234567)")
    assert p == {"exp_id": "K1-000000",
                 "name": "Rat IT Dose-Response (V1234567)",
                 "cro_study_id_guess": "V1234567"}


def test_parse_experiment_dirname_no_cro():
    p = _meta.parse_experiment_dirname("K1-000005 - ASO Design")
    assert p["exp_id"] == "K1-000005"
...
def test_parse_experiment_dirname_draft_and_separators():
    ...
    assert _meta.parse_experiment_dirname("K1-000006 — Immunotox Round 3")["exp_id"] == "K1-000006"
    # a parenthetical that's prose, not a code, is not mistaken for a study id
    p = _meta.parse_experiment_dirname("K1-000004 - F-Dup ASO Validation (Terminated)")
    assert p["cro_study_id_guess"] is None
...
def test_record_to_metadata_drops_empty_and_runtime():
    rec = {"exp_id": "K1-1", "name": "x", "tags": [], "cro": None, "note": "",
           "_page_count": 3, "document_id": "abc", "asos": ["ASO-7"]}
    meta = _meta.record_to_metadata(rec)
    assert meta == {"exp_id": "K1-1", "name": "x", "asos": ["ASO-7"]}


def test_experiment_card_deterministic_and_complete():
    rec = {"exp_id": "K1-000000", "title": "Rat IT Dose-Response",
           "cro": "Vendor A", "cro_study_ids": ["V1234567"],
           "status": "complete", "assays": ["Luminex", "QuantiGene"],
           "asos": ["ASO-7"], "related": ["K1-000002", "K1-000001"],
           "synopsis": "Three IT dose levels in rats."}
    a = _meta.experiment_card_markdown(rec)
    b = _meta.experiment_card_markdown(dict(rec))
    assert a == b                       # deterministic
    assert "K1-000000" in a
    assert "V1234567" in a
    assert "ASO-7" in a
    assert "Vendor A" in a
    # related list is sorted regardless of input order
    assert a.index("K1-000001") < a.index("K1-000002")
...
def test_catalog_markdown_escapes_pipes():
    # a stray pipe in any field must not corrupt the table (the K1-000007 bug)
